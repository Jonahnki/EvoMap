...
import {
  mockPathogenData as allPathogenData,
  mockOutbreakData as outbreakData,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pathogenResults = allPathogenData.filter(
      (pathogen) =>
    ...
    const outbreakResults = outbreakData.filter(
      (outbreak) =>
    ...
    if (filters) {
      ...
      if (filters.pathogens?.length) {
        ...
        filteredOutbreaks = filteredOutbreaks.filter((o) =>
          filters.pathogens.includes(o.name),
        );
      }
